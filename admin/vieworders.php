...
if (mysqli_fetch_array($qry) > 0) {
    ...
    <table border='3' align="center" id="customers" >
        ...
        <?php
        //  $qry = mysqli_query($conn,"select tbl_cmaster.orderid,tbl_customer.fname,tbl_customer.lname,tbl_customer.phone,tbl_cmaster.status from tbl_cmaster,tbl_customer where tbl_cmaster.custid=tbl_customer.id and tbl_cmaster.status='Order Placed'");  
         $qry = mysqli_query($conn,"select tbl_cmaster.orderid,tbl_customer.fname,tbl_customer.lname,tbl_customer.phone,tbl_cmaster.status from tbl_cmaster,tbl_customer where tbl_cmaster.custid=tbl_customer.id ORDER BY orderid ");  
        while ($row = mysqli_fetch_array($qry)) {
           
            echo '<tr><td>'  . $row['fname'] . ' '.  $row['lname'] .'</td><td>' . $row['phone'] . '</td><td><a href="vieworder2.php?id=' . $row['orderid'] . '" class="btn btn-success">view order</a></td></tr>';
